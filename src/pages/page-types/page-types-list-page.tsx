			...
			<div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
				<div className="space-y-1">
					...
					<p className="text-muted-foreground ml-[52px] text-sm">
						Tüm sayfa tiplerini görüntüleyin ve yönetin
					</p>
				</div>
				<Button
					...
					<div className="flex items-center gap-2">
						...
						<div>
							...
							<CardDescription className="text-xs">
								Sayfa tiplerini arayın ve filtreleyin
							</CardDescription>
						</div>
					</div>
				</CardHeader>
				...
				<CardHeader className="bg-gradient-to-r from-muted/50 via-muted/30 to-transparent border-b">
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-3">
							...
							<div>
								<CardTitle className="text-lg font-bold">Sayfa Tipi Listesi</CardTitle>
								<CardDescription className="text-xs mt-0.5">
									Toplam <span className="font-semibold text-foreground">{totalElements}</span> sayfa tipi bulundu
								</CardDescription>
							</div>
						</div>
					</div>
						...
						<Table>
							...
							<TableBody>
								{isLoading ? (
									...
									<TableRow>
										<TableCell colSpan={3} className="h-[400px] p-0">
					...
					<CardContent className="pt-6">
						<div className="flex flex-col sm:flex-row items-center justify-between gap-4">
							<div className="text-sm text-muted-foreground">
								Toplam <span className="font-bold text-foreground bg-primary/10 px-2 py-0.5 rounded">{totalElements}</span> sayfa tipi
								{" • "}
...
function PageTypeTableRow({ pageType, onView, onEdit, onDelete }: PageTypeTableRowProps) {
	return (
		<TableRow className="hover:bg-gradient-to-r hover:from-primary/5 hover:to-transparent transition-all duration-200 group border-b border-border/50">
			<TableCell className="font-bold text-primary/80">{pageType.id}</TableCell>
			<TableCell>
