			...
			{totalPages > 1 && (
				<div className="flex items-center justify-between">
					<div className="text-p3 text-muted-foreground">
						Toplam <span className="font-semibold text-foreground">{totalElements}</span> bileşen tipi
					</div>
...
// Component Type table row component
...
function ComponentTypeTableRow({ componentType, onView, onEdit, onDelete }: ComponentTypeTableRowProps) {
	...
	return (
		<TableRow className="hover:bg-muted/50 transition-colors">
			<TableCell className="font-medium">{componentType.id}</TableCell>
			<TableCell className="font-medium">{componentType.type}</TableCell>
			<TableCell>
				<div className="flex flex-wrap gap-1">
					{features.length > 0 ? (
						...
						<span className="text-p3 text-muted-foreground">Özellik yok</span>
					)}
				</div>
			</TableCell>
			<TableCell>
