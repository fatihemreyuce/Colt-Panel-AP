...
import { useUpdatePartner, useGetPartnerById, usePartners } from "@/hooks/use-partners";
...
export default function PartnerEditPage() {
	...
	const { data: partner, isLoading } = useGetPartnerById(partnerId);
		...
		return (
			<div className="flex-1 space-y-6 p-6 bg-gradient-to-br from-background via-background to-muted/20">
				<div className="text-center py-12">
					<p className="text-sm text-muted-foreground mb-4">Partner bulunamadı</p>
					<Button onClick={() => navigate("/partners")} className="bg-gradient-to-r from-primary to-primary/90 text-primary-foreground hover:from-primary/90 hover:to-primary shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 border-0">
						Geri Dön
					</Button>
				</div>
			</div>
				...
				<div className="bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border-b-2 border-border px-6 py-5">
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-sm text-muted-foreground mt-0.5">Partner bilgilerini güncelleyin</p>
						</div>
					</div>
				</div>
